...
namespace EDDTK.Plot3D.Rendering.View
{
	...
	public class View
	{
		...
		{
			EDDTK.Maths.Scale current = this.Scale;
			EDDTK.Maths.Scale newScale = current.@add(factor * current.Range);
		...
		public void Zoom(float factor, bool updateView)
		{
			...
			EDDTK.Maths.Scale scale = null;
			if ((zmin < zmax)) {
				scale = new EDDTK.Maths.Scale(zmin, zmax);
			} else {
				// forbid to have zmin = zmax if we zoom in
				if ((factor < 1)) {
					scale = new EDDTK.Maths.Scale(center, center);
				}
			}
			if ((scale != null)) {
		...
		public void ZoomX(float factor, bool updateView)
		{
			...
			EDDTK.Maths.Scale scale = null;
			if ((min < max)) {
				scale = new EDDTK.Maths.Scale(min, max);
			} else {
		...
		public EDDTK.Maths.Scale Scale {
			get { return new EDDTK.Maths.Scale(this.Bounds.zmin, this.Bounds.zmax); }
			set { setScale(value, true); }
		}

		public void setScale(EDDTK.Maths.Scale scale, bool notify)
		{
			BoundingBox3d bounds = this.Bounds;
			bounds.zmin = scale.Min;
			bounds.zmax = scale.Max;
		...
		public void RenderSceneGraph(bool light)
		{
			...
			Transform.Transform transform = new Transform.Transform(new EDDTK.Plot3D.Transform.Scale(_scaling));
